...
export default function FilterBar({ filters, onToggle, onReset, sorts, sortIdx, onSort }) {
  return (
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, flex: 1 }}>
          {sorts.map((s, i) => (
            <button
              ...
              style={{
                fontSize: 9, letterSpacing: 0.5, padding: '3px 7px',
                background: sortIdx === i ? S.accent + '2a' : S.surface2,
                color:      sortIdx === i ? S.accent : S.muted,
                border:     `1px solid ${sortIdx === i ? S.accent + '88' : S.border}`,
                borderRadius: 4, fontFamily: S.font, cursor: 'pointer', whiteSpace: 'nowrap',
              }}
            >
              {s.label}
